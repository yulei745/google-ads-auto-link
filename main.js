    ...
    async function getLink(link) {
        for(let i = 0; i < link.limit; i++){
            setTimeout(async () => {
                ...
                try {
                    const random_windowname = 'test'; //generate_random_str()
                    const body = {
                    ...
                    if(!find) {
                        bit.closeBrowser(browser_id);
                        utils.sleep(2000);
                        bit.deleteBrowser(browser_id);
                        return;
                    }

                    fs.appendFile('./output/landpage.txt', page.url()+'\n', 'utf8', (err) => {
                        if (err) {
                            console.error('追加失败:', err);
                        } else {
                            console.log('地址已记录，在 output/landpage.txt 查看');
                        }
                    });
